...
// arg0: id quien llama y buscara blanco de ataque

var ret = false;
if si_perseguidor(argument0.tipo) and random(1) < m_probact {
    var cercania = mu_vision;
    if argument0.tipo == mt_gigante {
        cercania *= 2; }
    var quien = noone;
    var tmp, solemne;
    with o_unidad {
        ...
        if estado != me_muerto and estado != me_guarecido and !solemne and
                propietario != argument0.propietario {
            tmp = point_distance(x, y, argument0.x, argument0.y);
            if tmp <= cercania {
                if si_objetivo(argument0.tipo, tipo) {
                    cercania = tmp;
                    quien = id; } } } }
    if quien != noone {
        if (argument0.tipo == mt_ciudadano or argument0.tipo == mt_turista) and actuar == 0 {
        ...
        with o_unidad {
            if estado == me_guarecido and propietario != argument0.propietario {
                tmp = point_distance(x, y, argument0.x, argument0.y);
                if tmp <= cercania {
                    if si_objetivo(argument0.tipo, tipo) {
                        cercania = tmp;
